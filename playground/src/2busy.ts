...
const POLL_PERIOD_MS: number = 500;
// static unsigned int HIGH_WATER_MARK_MS = 70;
const HIGH_WATER_MARK_MS: number = 70;
...
const AVG_DECAY_FACTOR: number = 3;
...
function Handle<T>(...args: any[]) {
    function TooBusy<T>(...args: any[]) {
        let block: boolean = false;
        let s_currentLag: number = 0;
        if (s_currentLag > HIGH_WATER_MARK_MS) {
            let pctToBlock = ((s_currentLag - HIGH_WATER_MARK_MS) / HIGH_WATER_MARK_MS) * 100.0;
            let r = (Math.random() / Math.max(s_currentLag, HIGH_WATER_MARK_MS) * 100);
